...
private:
  ...
  void check_data()
  {
    while ((vec_poses.size() != 0 && vec_lasers.size() != 0))
    {
      ...
      {
        gpflocalizer.updateData(vec_poses[0], vec_lasers[0]);
        vec_lasers.erase(vec_lasers.begin());
        vec_lasers_time.erase(vec_lasers_time.begin());
        vec_poses.erase(vec_poses.begin());
        vec_poses_time.erase(vec_poses_time.begin());
...
public:
  GpfNode(ros::NodeHandle &nh) : nh_(nh)
  {
    subscribe_laser = nh_.subscribe<sensor_msgs::LaserScan>("/laser/scan", 10, &GpfNode::callback_laser, this);
    subscribe_pose = nh_.subscribe<nav_msgs::Odometry>("/odom", 10, &GpfNode::callback_pose, this);
  }
};

int main(int argc, char **argv)
{
  ros::init(argc, argv, "gpf_cpp");
  ros::NodeHandle nh;
  GpfNode gpf_node(nh);
  ros::spin();
